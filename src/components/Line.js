...
const Line = function() {
    ...
    if (current_line){
      ...
      lines.forEach(element => {if (element.name === current_line){
          extra.name = element.displayName
          extra.color = element.color
      }})
    return (<div><div className='container'
    style= {{"gridTemplateColumns": (lines.length < 6) ? "repeat(" + lines.length + ", min(13vw, 80px))" :  "repeat(6, min(13vw, 80px))"}}>{arr}</div>
